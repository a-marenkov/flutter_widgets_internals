...
class CustomLayoutApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const _CustomLayoutPage(),
    );
  }
}

class _CustomLayoutPage extends StatefulWidget {
  const _CustomLayoutPage({
    Key? key,
  }) : super(key: key);

  @override
  State<_CustomLayoutPage> createState() => _CustomLayoutPageState();
}

class _CustomLayoutPageState extends State<_CustomLayoutPage> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(CustomLayoutApp.title),
      ),
      body: Center(
...
class EvenSidesWidgetsDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    layoutChild(middle, BoxConstraints.tight(middleSize));
